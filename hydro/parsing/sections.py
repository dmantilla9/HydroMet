...
def parse_department_and_commune(soup: BeautifulSoup, payload: dict) -> Tuple[Optional[str], Optional[str]]:
    dep = None
    dep_select = soup.find("select", {"name": "departement"})
    if dep_select:
        opt = dep_select.find("option", {"value": payload.get("departement")}) or \
              dep_select.find("option", selected=True) or dep_select.find("option")
        if opt: dep = opt.get_text(strip=True)

    com = None
    com_select = soup.find("select", {"name": "communeDepartement"})
    if com_select:
        opt = com_select.find("option", {"value": payload.get("communeDepartement")}) or \
              com_select.find("option", selected=True)
        if opt: com = opt.get_text(strip=True)

    if not com:
        h3 = soup.find(lambda t: t.name=="h3" and "Informations générales" in (t.get_text() or ""))
        if h3:
...
def parse_results_rows(soup: BeautifulSoup) -> List[dict]:
    ...
    for tr in table.find_all("tr")[1:]:
        ...
        if headers and len(vals)==len(headers):
            for k, v in zip(headers, vals): row[k]=v
        else:
            row = {
...
def extract_communes_block_csv(soup: BeautifulSoup) -> str:
    ...
    label = soup.find("label", string=lambda x: bool(x) and "Commune(s) et/ou quartier(s) du réseau" in x)
    if not label: return ""
